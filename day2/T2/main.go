...
func isGamePlayable(s string, b Bag) int {
	...
	for _, GameTurn := range GameRounds {
		...
		for _, Pick := range GamePicks {
			...
			if BallType == "blue" {
				currMin.blue = max(BallCount, currMin.blue)
			} else if BallType == "green" {
				currMin.green = max(BallCount, currMin.green)
			} else if BallType == "red" {
				currMin.red = max(BallCount, currMin.red)
			} else {
	...
	return currMin.red * currMin.blue * currMin.green
}
